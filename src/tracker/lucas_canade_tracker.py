...
class LucasKanadeTracker:
    def __init__(self):
        ...
        self.__lk_params = dict(winSize=(15, 15), maxLevel=5, criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT,
                                30, 0.001))

    def detect(self, image):
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        update_points, status, _ = cv2.calcOpticalFlowPyrLK(self.__old_gray_frame, gray, np.array(self.__lk_points,
